...
type canonicalReceipt struct {
	...
	EnergyEstimateJoules       float64 `json:"energy_estimate_joules,omitempty"`
...
func (r *Receipt) CanonicalBytes() ([]byte, error) {
	cr := canonicalReceipt{
		CarbonIntensityGCO2ePerKWh: r.CarbonIntensityGCO2ePerKWh,
		CertificatePEM:             r.CertificatePEM,
		DiscrepancyPct:             r.DiscrepancyPct,
		EnergyEstimateJoules:       r.EnergyEstimateJoules,
		EnergySource:               r.EnergySource,
		ID:                         r.ID,
		InferenceRegion:            r.InferenceRegion,
